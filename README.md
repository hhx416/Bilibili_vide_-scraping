...
4. 自动验证 Cookie 有效性
5. 搜索视频并提取 BV 号
6. 按每进程 2 个视频分配任务，启动多进程并发下载
7. 每个视频默认最高画质，自动下载视频+音频
8. FFmpeg 自动合并为 MP4，文件名带发布时间
...
```
output/
├── video/                                          # 视频分片（合并成功后自动删除）
│   ├── [2024-01-15] 视频标题1.m4s
│   └── [2024-01-16] 视频标题2.m4s
├── audio/                                          # 音频分片（合并成功后自动删除）
│   ├── [2024-01-15] 视频标题1.m4a
│   └── [2024-01-16] 视频标题2.m4a
...
A: 支持！使用 `bsps.py` 搜索批量下载脚本：
- 输入关键词自动搜索视频
- 多进程并发下载（每进程 2 个视频）
- 自动合并，文件名带发布时间
- 详见 [搜索批量下载（bsps.py）](#搜索批量下载bspy)

番剧和单视频脚本（bsp.py / bspv.py）只支持单集/单视频下载。
...
### Q: bsps.py 的两个版本有什么区别？

A: 功能完全一样，区别只是注释多少：
- `bsps.py`：精简版，无多余注释，适合日常使用
- `bsps - 副本.py`：详细注释版，适合学习代码

### Q: 搜索下载为什么要有请求间隔？

A: 为了模拟真实用户行为，降低触发 B 站反爬机制的风险。
每个视频之间间隔 1-2 秒，虽然慢一点但更稳定。
...
8. **流式下载**：`stream=True` 分段下载，实时显示进度
9. **音视频合并**：调用 FFmpeg 无损合并（`-c copy`）
10. **多进程并发**：`multiprocessing.Pool` 进程池，每进程 2 个任务
